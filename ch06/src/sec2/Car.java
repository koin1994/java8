...
public class Car {
	...
	final static int MIN_SPEED = 0; //멤버상수는 반드시 초기화 되어야 하며, 그값은 바뀌지 않음
	//[접근제한자] [수정자] 데이터타입 필드명;
	...
	//getter Method(개터 메서드) : 현재 객체의 속성(멤버 필드)에 저장된 데이터 값을 가져오기
	//setter Method(세터 메서드) : 현재 객체의 속성(멤버 필드)에 값을 저장하기
	
	//return (반환값)이 없으므로 변환타입에는 void를 지정해야z함
	public void bootUp() {
	...
	public int speedUp(){   //반환 타입이 int 이므로 return 해주어야 하는 것은 정수 (int)이어야 함

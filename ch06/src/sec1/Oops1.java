...
public class Oops1 {
	public static void main(String[] args) {
		...
		//클래스와 같은 구조(툴)로 만든 인스턴스 -> 객체
		Member mem = new Member();
		//이때 Member는 클래스명, mem는 인스턴스(객체), new는 객체를 만드는 명령(키워드), 
		//Member() 는 생성자(Constructor)
		//인스턴스(객체)는 자바에서 1)기본으로 제공하는 클래스이거나 2)사용자가 필요에 따라 만든 클래스로 부터 생성될 수 있다. 
		...
		//상속이 될때 상속을 해주는 클래스를 슈퍼 클래스라하고, 상속을 받는 클래스는 서브 클래스라 함.
		//상속 후에 메서드나 생성자 함수를 여러 가지 형태로 다르게 바꿀 수 있는데 이를 메서드 오버라이딩과 
		//하나의 클래스 내에서 매서드나 함수의 매개변수 개수나 변수데이터타입에 따라 같은 이름으로 여러 개 지정이 가능한 데 오버로딩
		//객체를 만들 때 여러 형태로 만들거나 선언할 수도 있고, 메서드 오버라이딩이나 메서드 오버로딩과 같이 같은 이름의 여러 형태로 활용할 수 있는데
		//이러한 것을 -> 다형성
		//추상화 - 부모클래스(인터페이스)에서는 간소화된 꼭 필요한 요소만으로 구성하여 다른 서브 클래스에서 
